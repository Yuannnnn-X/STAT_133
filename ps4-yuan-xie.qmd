...
In Problem Set 2, we used `cbind()` to combine these vectors into a matrix. Now, use `data.frame()` to combine the atomic vectors above into a dataframe called `HEROES` with the columns `name`, `age`, `height`, `weight`, and `strength`.
...
```{r}
HEROES_new <- HEROES |> select(name, strength)
HEROES_new
```
...
Your answer here: row.names. NULL or a single integer or character string. Yes because it is an integer. It means that the dataframe has 3 rows with default row names "1", "2", and "3".
...
HEROE_1 <- data.frame(
  ...
  weight,
  strength,
  row.names = name # set the row names here
)
...
HEROE_2 <- data.frame(
  ...
  height = height,
  weight = weight,
  strength = strength, # explicitly specify the columns
  row.names = name # set the row names here
)
...
```{r}
dat |> slice(seq(from = 10, to = 50, by = 5))
```
...
```{r}
dat |> filter(manufacturer == "honda") |> select(model, trans, cty)
```
...
```{r}
dat |> select(manufacturer, model, hwy, fl) |> filter(hwy >= 35)
```
...
```{r}
dat |> filter(class == "compact", cyl == 6) |> select(manufacturer, model, trans)
```
...
```{r}
dat |> group_by(manufacturer) |> summarize(mean_cty = mean(cty))
```
...
```{r}
dat |> mutate(cmpg = cty * 0.55 + hwy * 0.45)
```
...
```{r}
dat |> group_by(class) |> summarize(max_cty = max(cty)) |> arrange(class)
```
...
Write a function that takes in `dat` and additional inputs and uses `dplyr` verbs to do each of the following:
...
```{r}
subset_dat <- function(dat, manu, max_displ) {
  # check if manufacturer exists (if the manufacturer is not included)
  if (!manu %in% dat$manufacturer) {
    warning("Manufacturer not found in dataset.")
    return(NULL)
  }
  
  dat |> filter(manufacturer == manu, displ < max_displ)
}
```
...
```{r}
subset_dat_hwy <- function(dat, class_type, min_hwy, max_hwy) { # two values specified
  # check if class exists (if the class is not included)
  if (!class_type %in% dat$class) {
    warning("Class not found in dataset.")
    return(NULL)
  }
  
  dat |> filter(class == class_type, between(hwy, min_hwy, max_hwy))
}
```
...
```{r}
avg_col <- function(dat, col) {
  # check if column is numeric (if the class is not numeric)
  if (!is.numeric(dat[[col]])) {
    warning("Selected column is not numeric.")
    return(NULL)
  }
  
  dat |> group_by(manufacturer, class) |> summarize(avg_value = mean(dat[[col]]))
}
```
