...
#' @return c length of the hypotenuse
pythagoras <- function(a, b) {
  c <- sqrt(a ^ 2 + b ^ 2)
  return(c)
}
...
gaussian <- function(x, mu, sigma) {
  if (sigma <= 0) {
    # raises an error if sigma is not positive with stop()
    stop("Error: sigma must be positive.")
  } else {
  g <- exp((((x - mu) / sigma)^2) / (-2)) / (sigma * sqrt(2 * pi))
  return(g)
  }
...
# plot
plot(x, y, type = "l",
     main = "Normal Curve (mu = 0, sigma = 1)",
     xlab = "x",
     ylab = "g(x)")
```
...
Your description here: `dnorm()` includes an additional argument log = TRUE or FALSE . It computes the logarithmic value of the probability density. It is useful when the probability density is quite small.
...
```{r}
# write your function here
logistic_growth <- function(capacity = 5000, initial = 100, rate = 0.5, time = 10) {
  population <- capacity / (1 + ((capacity - initial) / initial) * exp(-rate * time))
  return(population)
}
```
...
```{r}
# call your function here
logistic_growth(capacity = 5000, initial = 100, rate = 0.4, time = 10)
```
...
```{r}
# call your function here
logistic_growth(capacity = 2000, initial = 250, rate = 0.25, time = 15)
```
...
```{r}
# call your function here
rate_vec <- c(0.15, 0.25, 0.35)
logistic_growth(capacity = 2000, initial = 250, rate = rate_vec, time = 15)
```
...
#' @param p Numeric. Probability of success in a single trial (0 ≤ p ≤ 1).

bin_probability <- function(n = 10, k = 5, p = 0.4) {
  # error checks
  if (n < 1) {
